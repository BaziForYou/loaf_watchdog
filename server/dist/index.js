...
var require_fsevents_handler = __commonJS({
  "node_modules/.pnpm/chokidar@3.6.0/node_modules/chokidar/lib/fsevents-handler.js"(exports2, module2) {
    ...
    var calcDepth = (path2, root2) => {
      let i = 0;
      while (!path2.indexOf(root2) && (path2 = sysPath.dirname(path2)) !== root2) i++;
      return i;
    };
...
var separator = import_path.default.sep;
var delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
async function restartResource(resourceName) {
  StopResource(resourceName);
  await delay(250);
  StartResource(resourceName);
}
var currentResource = GetCurrentResourceName();
var resourcePath = GetResourcePath(currentResource);
var resourcesIndex = resourcePath.lastIndexOf("/resources");
var root = resourcePath.slice(0, resourcesIndex) + "/resources";
var config = JSON.parse(LoadResourceFile(currentResource, "config.json"));
var debouncedRestart = null;
import_chokidar.default.watch(`${root}/**/*.lua`, {
  persistent: true,
  ignored: ["**/node_modules", "**/.git", "**/ui", "**/html", "**/uploads"],
  ignoreInitial: true
}).on("all", async (event, path2) => {
  const parts = path2.replace(`${root}/`.replaceAll("/", separator), "").split(separator).filter((part) => part !== "resources" && part[0] !== "[" && part[part.length - 1] !== "]");
  const resourceName = parts[0];
  if (!resourceName) {
    return console.log(`^1[ERROR]^7 Could not find resource name for ${path2}`);
  }
